...
const List = ({ loading, calcList, summary, user, status, requestList, setTitle, setAdd, setModal, setContent }) => {
  useEffect(() => {
    setTitle('履歴')
    setAdd(true)
    requestList()
        ...
        return (
          <div>
            ...
            <div>
              {status.type === 'solo'
                ? status.rate
                : user.userKey === status.host
                ? status.rate
                : 100 - parseInt(status.rate)}
              <span>%</span>
            </div>
          </div>
        )
        ...
        <details>
          <summary>
            <div>
              ...
              <div>
                <div>
                  {lib.getSymbol(summary[selfType] - charge[selfType]) +
                    lib.addSeparator(Math.abs(summary[selfType] - charge[selfType]))}
                  <span>円</span>
                </div>
          ...
          <div>
            <div>
              <label>支払計</label>
              <div>
                {lib.addSeparator(summary.payment)}
                <span>円</span>
              </div>
            </div>
            <div>
              <label>{selfName}の支払計</label>
              <div>
                {lib.addSeparator(summary[selfType])}
                <span>円</span>
              </div>
            </div>
            <div>
              <label>{otherName}の支払計</label>
              <div>
                {lib.addSeparator(summary[otherType])}
                <span>円</span>
              </div>
            </div>
            <div>
              <label>{selfName}の負担</label>
              <div>
                {lib.addSeparator(charge[selfType])}
                <span>円</span>
              </div>
            </div>
            <div>
              <label>{otherName}の負担</label>
              <div>
                {lib.addSeparator(charge[otherType])}
                <span>円</span>
              </div>
            </div>
            {showRate()}
  ...
  const showList = () => {
    ...
    const otherName = status ? status.othername : ''
    const selfType =
      status.type === 'solo' ? 'hostPayment' : status.host === user.userKey ? 'hostPayment' : 'clientPayment'
    const otherType =
      status.type === 'solo' ? 'clientPayment' : status.host === user.userKey ? 'clientPayment' : 'hostPayment'
    return (
      <ol className="list">
        {Array.from(calcList.keys()).map((eachDay, i) => {
          ...
          const listEachDay = calcList.get(eachDay).map((content, j) => {
            const date =
              content.useDate === 'true' ? (
                <div className="date-disable">
                  <span>00:00</span>
                  <span>なし</span>
                </div>
              ) : (
                <div className="date">{lib.unixTime(content.sendDate)}</div>
              )
            ...
            const paid =
              Number(content[selfType]) === 0 || Number(content[otherType]) === 0 ? (
                Number(content[selfType]) !== 0 ? (
                  <div className="paid self"></div>
                ) : (
                  <div className="paid other"></div>
                )
              ) : (
                <div className="paid pair" style={payPercent}></div>
              )
            return (
              <li key={'list' + i + j} onClick={() => openModal(content)} onTouchStart={() => {}}>
                ...
                <div className="payment">
                  {lib.addSeparator(parseInt(content.payment))}
                  <span>円</span>
                </div>
              </li>
            )
