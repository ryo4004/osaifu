...
const Rate = ({ rate, user, status, changeRate, requestChangeRate, setTitle, setBack }) => {
  useEffect(() => {
    setTitle('負担率の変更')
    setBack('/setting')
    if (status && user)
      changeRate(
        status.type === 'solo' ? status.rate : status.host === user.userKey ? status.rate : 100 - parseInt(status.rate)
      )
    return () => {
      setBack(false)
    }
