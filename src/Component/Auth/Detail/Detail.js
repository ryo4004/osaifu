...
const Detail = ({ modal, content, user, status, setModal, requestDelete }) => {
  const showDelete = (id) => {
    confirmAlert({
  ...
  const showContent = () => {
    ...
    const time =
      content.useDate === 'true' ? (
        ...
        <span className="time">{date.split('T')[1].split(':')[0] + ':' + date.split('T')[1].split(':')[1]}</span>
      )
    ...
    const selfPayment =
      Number(content[selfType]) === 0 ? (
        ...
        <div className="self-payment">
          ...
          <div>
            {lib.addSeparator(Number(content[selfType]))}
            <span>円</span>
          </div>
        </div>
      )
    const otherPayment =
      Number(content[otherType]) === 0 ? (
        false
      ) : (
        <div className="other-payment">
          <label>{otherName}</label>
          <div>
            {lib.addSeparator(Number(content[otherType]))}
            <span>円</span>
          </div>
        </div>
      )
    return (
      <div>
        <div className="date">
          <div>
            <span className="day">{day.replace(/-/g, '/')}</span>
            {time}
          </div>
        </div>
        <div className="payment">
          <label>支払額</label>
          <div>
            {lib.addSeparator(Number(content.payment))}
            <span>円</span>
          </div>
        </div>
        {memo}
    ...
    <div className="detail">
      <div className={'modal-contents' + modalClass}>
        <div className="detail-content">
          ...
          <div className="modal-content">{showContent()}</div>
        </div>
      </div>
